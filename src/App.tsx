...
import {
  rememberOrder, setOrderPayment, fetchMessageStats, adminDashboard, adminDelete, adminDeleteOrder, adminList, adminSaveMenuItem, adminUpsert, createChef, createOrder, deleteChef, updateChef,
...
function Login({ role }: { role: 'admin' | 'chef' }) {
  const nav = useNavigate(); const [email, setEmail] = useState(role === 'admin' ? 'jemuvalos@gmail.com' : ''); const [password, setPassword] = useState(''); const [busy, setBusy] = useState(false); const [error, setError] = useState('');
  const submit = async (e: React.FormEvent) => { e.preventDefault(); setBusy(true); setError(''); try { if (!supabase) { if (role === 'admin' && email.trim().toLowerCase() !== 'jemuvalos@gmail.com') throw new Error('Demo admin uses jemuvalos@gmail.com.'); sessionStorage.setItem('bitecraft:role', role === 'admin' ? 'admin' : 'chef'); if (role === 'chef') { const em = email.trim().toLowerCase(); sessionStorage.setItem('bitecraft:demo-role', em.includes('manager') ? 'manager' : em.includes('kitchen') ? 'kitchen_staff' : 'chef'); sessionStorage.setItem('bitecraft:demo-id', `demo-${em || 'chef'}`); sessionStorage.setItem('bitecraft:demo-name', (em.split('@')[0] || 'Chef').replace(/^./, c => c.toUpperCase())); } nav(role === 'admin' ? '/admin/dashboard' : '/chef/dashboard'); return; } const { error } = await supabase.auth.signInWithPassword({ email, password }); if (error) throw error; const actual = await currentRole(); if (role === 'admin' && actual !== 'admin') throw new Error('This account does not have administrator access.'); if (role === 'chef' && !['chef', 'kitchen_staff', 'manager', 'admin'].includes(actual || '')) throw new Error('This account does not have staff access.'); nav(role === 'admin' ? '/admin/dashboard' : '/chef/dashboard'); } catch (err: any) { setError(err?.message || 'Login failed.'); } finally { setBusy(false); } };
  return <div className="auth-page"><form className="auth-card" onSubmit={submit}><Link to="/" className="brand"><BrandLogo /></Link><span className="eyebrow">{role === 'admin' ? 'ADMIN PORTAL' : 'STAFF PORTAL'}</span><h1>{role === 'admin' ? 'Welcome back.' : 'Staff login.'}</h1><p>{role === 'admin' ? 'Manage the restaurant, staff, orders and website.' : 'Chefs, kitchen staff and managers sign in here.'}</p><div className="field"><label>Email</label><input type="email" required value={email} onChange={e => setEmail(e.target.value)} placeholder="name@restaurant.com" /></div><div className="field"><label>Password</label><input type="password" required minLength={4} value={password} onChange={e => setPassword(e.target.value)} placeholder="••••••••" /></div>{error && <div className="error">{error}</div>}<button className="btn dark wide" disabled={busy}>{busy ? 'SIGNING IN…' : 'SIGN IN'} <ArrowRight size={16} /></button><span className="demo-note">{hasSupabase ? 'Use the credentials created in Supabase Authentication.' : 'Demo mode: this portal is available locally. Use an email containing "manager" or "kitchen" to try those roles. Supabase becomes the real authentication layer after you add the environment variables and run the SQL setup.'}</span></form></div>;
}
